            ...
            case(state)
                ...
                READ_DATA : begin
                   ...
                   state_read_data <= {state_read_data[6:0],miso};     
                              /*      
                    state_read_data[count_bit] <= miso;
                              if(count_bit ==0) begin 
                                 count_bit =7;
                                 */
                                 next_state =STOP;  
...
module SPI_ADDR_DATA11(
    ...
    always @(posedge clk or posedge reset_p) begin
        if (reset_p) begin 
            count_5ns = 0;
            sck = 1;
        end else if (sck_clk_enable) begin
            if (clk) begin
                if (count_5ns >= 4) begin
                    count_5ns = 0;
                    sck = ~sck;
                end else begin
                    count_5ns = count_5ns + 1;
                end
            end
        end else begin
            sck = 1;
    ...
    parameter INIT = 7'b000_0010; // 초기화 상태
    parameter COUNT_ADDR_DATA = 7'b000_0100;
    parameter SEND_ADDR = 7'b000_1000; // 클럭 신호
    parameter SEND_DATA = 7'b001_0000;
    parameter READ_DATA = 7'b010_0000;
    parameter STOP = 7'b100_0001;
        ...
        if (reset_p) begin
            ...
            case (state)
                IDLE: begin
                    sck_clk_enable = 0;
                    if (btn_pedge) begin
                        next_state = INIT; // 버튼 눌림 -> 초기화 상태로 전환
                    end
                end    
                
                INIT: begin
                    // MFRC522 초기화 명령 전송
                    addr <= modereg_addr; // 모드 레지스터 주소
                    data <= modereg_data; // 모드 레지스터 데이터
                    next_state = COUNT_ADDR_DATA; // 주소 전송 상태로 전환
                end
    ...
    wire [7:0] value_data;   
    bin_to_dec adc_bcd(.bin(received_data), .bcd(value_data)); // 0부터 1023까지
    fnd_cntr(.clk(clk), .reset_p(reset_p), .value(value_data), .com(com), .seg_7(seg_7));    
