...
module SPI_TEST_TOP2(
    ...
    SPI_ADDR_DATA data(.clk(clk),.reset_p(reset_p),.miso(miso), .irq(irq),
                                 .btn(btn[1]), .sck(sck) , .mosi(mosi),
                                 .received_data(received_data),.led_debug(led_debug));
   

    wire [7:0] spi_data;
    bin_to_dec tmpr(.bin(received_data), .bcd(spi_data));
    
     fnd_cntr (.clk(clk), .reset_p(reset_p), .value(spi_data), .com(com), .seg_7(seg_7));
