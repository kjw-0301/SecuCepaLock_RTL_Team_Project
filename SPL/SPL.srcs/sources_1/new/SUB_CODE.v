...
module I2C_controller(
   ...
   edge_detector_n COMM_GO_P(.clk(clk), .reset_p(reset_p), .cp(comm_go),  
    ...
    always @(posedge clk or posedge reset_p) begin
        ...
        else if(scl_e)begin         //scl_e는 scl 이 1일만 동작 
            if(clk_usec) begin
...
module I2C_lcd_send_byte(
    input clk,reset_p,
    input [6:0] addr,
    input rs, send,     //send가 1일 comm_go에 1려서 상위 4비트 보내고/ enable 하위 4비트보내고 /enable
            ...
            case(state)
                 ...
                 SEND_DISABLE : begin
                    if(count_usec <= 22'd200)  begin  //200us //start, I2C과정(18)  , stop 해서 여유롭게 200us
                        ...
                        comm_go =0;
                        next_state = IDLE;   
                        busy =0;    //IDLE로 갈 busy가 1 , 나머지에선 0
