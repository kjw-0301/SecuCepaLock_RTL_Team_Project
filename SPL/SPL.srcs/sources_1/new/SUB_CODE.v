...
module ultrasonic_sensor_cntr(
    ...
    edge_detector_n ed13(.clk(clk), .reset_p(reset_p), .cp(echo1), 
         ...
         reg[21:0] count_usec;  //3Ï¥? Ïπ¥Ïö¥?ä∏?ïòÍ∏? ?úÑ?ï¥ 22ÎπÑÌä∏
         reg count_usec_en;   //enable
         
         reg[21:0] echo_time;   //?Çò?àó?Öà Ï≤òÎ¶¨Î•? ?úÑ?ï¥ ?ãúÍ∞ÑÏùÑ ???û•?ï† Î≥??àò
    ...
    always @(negedge clk or posedge reset_p)begin
          if(reset_p)begin
            ...
            S_IDLE : begin
                if(count_usec < 22'd1_000_000)  begin   //1Ï¥?
                   count_usec_en = 1;
            ...
            ECHO_L : begin
                if(echo_n_edge) begin
                    count_en =0;
                    distance = cm;      //distance Í∞íÏùÑ valueÍ∞íÏóê ?Ñ£?óà?úº?ãàÍπ? cm
     ...
     //?Çò?àó?Öà ?†ú?ñ¥(Î®πÏä§Î°? Íµ¨ÏÑ±) 
     always @(posedge clk or posedge reset_p)begin
        if(reset_p)distance =0;
        else begin  //58?ùò Î∞∞Ïàò
